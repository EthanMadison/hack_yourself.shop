...
def get_cart() -> dict:
    ...
    Возвращает:
        dict: Словарь вида '{str(product_id): int(quantity)}' с количеством каждого товара в корзине.
    """
...
def save_cart(cart: dict) -> None:
    ...
    Параметры:
        cart: Словарь вида '{str(product_id): int(quantity)}', представляющий содержимое корзины.
